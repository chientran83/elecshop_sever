        ...
        mounted(){
            ...
            if(this.get_cookie){
                fetch('http://localhost:8000/api/v1/users/user_login',{headers:{"Authorization" : "Bearer " + this.get_cookie,'Content-Type': 'application/json','Accept': 'application/json'}})
                    ...
                    .then(()=>{
                        // fetch data order
                        fetch('http://localhost:8000/api/v1/order/index/'+this.record_number).then(res => res.json()).then(res => {
        ...
        methods:{
            load_data_order:function(url){
                if(url != ''){
                    ...
                    var link = 'http://localhost:8000/api/v1/order/index/' + this.record_number;
                }
            ...
            delete_order:function(id){
                ...
                if (result.isConfirmed) {
                    axios.delete('http://localhost:8000/api/v1/order/'+id,{headers:{"Authorization" : "Bearer " + this.get_cookie}})
                    .then(res => {
                        Swal.fire(
                        'Deleted!',
